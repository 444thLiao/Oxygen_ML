...
likelihood = open('likelihoods.txt').readlines()[1:]
print('Families included: ', len(likelihood))

fams_used = [_.split('\t')[0] for _ in likelihood]
...
typelist = []
for i in range(1,len(likelihood[0].split())-1):
...
abe = 0
for i in tqdm(range(1,len(parse_results))):
    abe += 1
    print(abe, i)
	...
	#print obscount, fams[obscount], families.count(fams[obscount])
	    ...
	    #print fams[obscount]
            words = likelihood_freqs.strip().split('\t')
                   
            if float(obs) > 0:     #p_obs_hyp = p(precense/absence-observationer|hyp)
                count = 0
